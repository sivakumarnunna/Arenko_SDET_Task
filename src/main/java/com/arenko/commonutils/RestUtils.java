...
public class RestUtils {
	...
	public String getAuthorizationToken() {
		if (authToken == null || authToken.length() < 1) {
			JSONObject jsonObj = new JSONObject().put("username", ApplicationConstants.USER_NAME)
					.put("password", ApplicationConstants.PASSWORD);
			
			RequestSpecification rspec = getCommonSpec().basePath("auth")
					.body(jsonObj.toString());

			Response response = sendRequest(rspec, ApplicationConstants.POST_REQUEST, null);
